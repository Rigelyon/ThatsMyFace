...
def display_face_encrypt_test():
    ...
    plaintext = st.text_area("Enter text to encrypt:", height=100, value="Omke gams omke gams!")
        ...
        with st.spinner("Running test..."):
            ...
            if embedding1 is None or embedding2 is None:
                ...
                if similarity < MIN_SIMILARITY_THRESHOLD:
                    ...
                    st.warning("Authentication may fail regardless of tolerance setting")
                elif similarity < SIMILARITY_THRESHOLD:
                    st.warning(f"⚠️ Face Similarity: {similarity:.4f} - These faces have low similarity")
                    st.info("You may need higher tolerance settings to decrypt successfully")
                ...
                if similarity < MIN_SIMILARITY_THRESHOLD:
                    ...
                    st.info("The system won't allow decryption when faces don't meet the minimum similarity threshold.")
                    return
                ...
                similarity_factor = max(0.0, min(1.0, (similarity - MIN_SIMILARITY_THRESHOLD) /
                ...
                if decrypted_data is not None:
                    ...
                    if key2 is None or key2 == b'':
                        st.error("❌ TEST FAILED: Could not regenerate the key from the second face. This usually means the face images are too different, or the error tolerance is too low. Try increasing the error tolerance or using more similar images.")
                    else:
