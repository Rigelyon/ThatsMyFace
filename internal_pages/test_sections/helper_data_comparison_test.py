...
def display_helper_data_comparison_test():
    ...
    with col1:
        ...
        face1_file = st.file_uploader(
            "Upload first face photo", type=["jpg", "jpeg", "png"], key="helper_face1"
        )
        if face1_file:
    ...
    with col2:
        ...
        face2_file = st.file_uploader(
            "Upload second face photo", type=["jpg", "jpeg", "png"], key="helper_face2"
        )
        if face2_file:
    ...
    if st.button("Run Comparison Test", disabled=not (face1_file and face2_file)):
        with st.spinner("Running test..."):
            ...
            if embedding1 is None or embedding2 is None:
                ...
                start_time = time.time()
                key1, helper_data1 = generate_key_with_helper(
                    embedding1, user_tolerance
                )
                key2, helper_data2 = generate_key_with_helper(
                    embedding2, user_tolerance
                )
                ...
                common_keys = set(helper_data1.keys()).intersection(
                    set(helper_data2.keys())
                )

                for key in common_keys:
                    if isinstance(helper_data1[key], (int, float)) and isinstance(
                        helper_data2[key], (int, float)
                    ):
                        value1 = helper_data1[key]
                        value2 = helper_data2[key]
                        if key == "error_tolerance":
                            helper_data_differences[key] = (
                                "Same" if value1 == value2 else f"{value1} vs {value2}"
                            )
                        elif key in ["vector_mean", "vector_std"]:
                            diff = abs(value1 - value2)
                            relative_diff = (
                                diff / max(abs(value1), abs(value2), 1e-10) * 100
                            )
                            helper_data_differences[key] = (
                                f"{relative_diff:.2f}% difference"
                            )
                    elif key == "vector_shape":
                        helper_data_differences[key] = (
                            "Same"
                            if helper_data1[key] == helper_data2[key]
                            else "Different"
                        )

                # Cross-regeneration test (try to regenerate key1 using embedding2 with helper_data1)
                can_cross_regenerate1 = (
                    regenerate_key_from_helper(embedding2, helper_data1) == key1
                )
                can_cross_regenerate2 = (
                    regenerate_key_from_helper(embedding1, helper_data2) == key2
                )
                ...
                if similarity > SIMILARITY_THRESHOLD and key_match_percentage > 90:
                    ...
                    similarity > MIN_SIMILARITY_THRESHOLD and key_match_percentage > 70
                ):
                    st.warning(
                ...
                st.info(
                    f"Key generation with helper data time: {key_generation_time:.4f} seconds"
                )
